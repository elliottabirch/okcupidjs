...
class OKCupid {
  ...
  paginatedSearch(query) {
    ...
    rx.Observable.from(bodyTypes)
          ...
          .mergeMap(plainuser => rx.Observable.fromPromise(this.getUserProfile(plainuser.username))
            ...
            .map(user =>
              rx.Observable
                ...
                .map(body => [body, user]))
            .mergeAll(2)
            .do(([body, user]) => { if (body.pagination.last < 50) { datamap.lowquestions += 1; log.error(`${user.username} only has ${body.pagination.last} messages, so skipping`); } })
      ...
      .subscribe(([bodyType]) => { }, (err) => { log.error(err.stack); logDataMap(datamap); }, () => {
        logDataMap(datamap);
      });
  }
}
