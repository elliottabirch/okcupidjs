const { URLS, locationNames, locations, bodyTypes } = require('../custom_modules/constants');
...
class OKCupid {
  ...
  paginatedSearch(query) {
    ...
    rx.Observable
      .from(locationNames)
      .mergeMap(locationName => rx.Observable.from(bodyTypes)
        .mergeMap((bodyType) => {
          query.location = locations[locationName];
          query.bodytype = [bodyType];
          log.error('-----------starting--------------\n');
          log.error(`${locationName}-${bodyType}\n`);
          log.error(`${JSON.stringify(query)}\n`);
          resetDataMap();
          return rx.Observable
            .fromPromise(this.betterSearch(query))
            ...
            .mergeMap(plainuser => rx.Observable.fromPromise(this.getUserProfile(plainuser.username))
              .do((user) => { datamap.miscErrors += 1; return user.status !== 1 && log.error(`unable to find profile for user ${user.username}`); })
              .skipWhile(user => user.status !== 1)
              .map(user =>
                rx.Observable
                  .fromPromise(this.findDbUserByUsername(user.username))
                  .do((dbUser) => { datamap.alreadyExists += 1; return dbUser && log.error(`user ${dbUser.username} already exists`); })
                  .map(db => [db, user]))
              .mergeAll(2)
              .skipWhile(([db]) => db)
              .map(([, user]) => user)
              .map(user =>
                rx.Observable
                  ...
                  .map(body => [body, user]))
              .mergeAll(2)
              .do(([body, user]) => { datamap.lowquestions += 1; return body.pagination.last < 50 && log.error(`${user.username} only has ${body.pagination.last} messages, so skipping`); })
              .filter(([body]) => body.pagination.last > 50)
              .map(([body, user]) => rx.Observable
                ...
                .map(() => rx.Observable
                  ...
                  .map(timesToRepeat => rx.Observable
                    ...
                    .mergeMap(low => rx.Observable.fromPromise(this.getUserQuestions(user.username, low))
            ...
            .map(() => [locationName, bodyType]);
        }, null, 1), null, 1)
      .subscribe(([locationName, bodyType]) => { datamap.newUsersCreated += 1; log.error(`finished ${locationName}-${bodyType}`); }, err => log.error(err.stack), () => {
        log.error(`new users: ${datamap.newUsersCreated}\n`);
        log.error(`lowquestions: ${datamap.lowquestions}\n`);
        log.error(`alreadyExists: ${datamap.alreadyExists}\n`);
        log.error(`newQuestionsCreated: ${datamap.newQuestionsCreated}\n`);
        log.error(`miscErrors: ${datamap.miscErrors}\n`);
        log.error('----------------------------\n');
      });
  }
}
